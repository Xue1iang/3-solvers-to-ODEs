...
%% Deal with NAN after ODE45 solver
...
t = linspace(0, 402.2, n);

% integrate the yaw rate to get yaw angle
yawAngle = cumtrapz(t, fr); 
...
x_ = cos(yawAngle) .* fu - sin(yawAngle) .* fv;    % x_velocity in earth-fixed frame
y_ = sin(yawAngle) .* fu + cos(yawAngle) .* fv;    % y_velocity in earth-fixed frame
...
t1 = linspace(0, tsf(end), length(fu));
...
legend( 'measured velocity', 'ode45_solver velocity')
...
hold on
plot(positionGlobalX, positionGlobalY,'b-')
